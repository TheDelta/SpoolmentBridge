# Copy of https://github.com/prusa3d/OpenPrintTag/blob/main/utils/nfc_initialize.py | but only returns the payload for SpoolmentBridge
...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'opt/utils')))
...
def nfc_initialize(args: Args):
    ...
    # NOTE change instead of full_data only payload | export as base64
    return json.dumps([config.mime_type, base64.b64encode(payload).decode('ascii'), base64.b64encode(full_data).decode('ascii')])
...
if __name__ == "__main__":
    ...
    # NOTE return string instead of buffer
    sys.stdout.write(nfc_initialize(parser.parse_args().args))